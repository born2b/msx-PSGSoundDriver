...
SOUNDDRV_INIT_2:
    ...
    ; ■ エンベロープ初期化
    LD HL, SOUNDDRV_ENVELOPEFLAT
    XOR A
    CALL SOUNDDRV_REGENVELOPE
    INC A
    CALL SOUNDDRV_REGENVELOPE
    INC A
    CALL SOUNDDRV_REGENVELOPE
...
SOUNDDRV_INITWK_L1:
    ...
    LD E,(HL)						; BC <- エンベロープID:0のアドレスから求めたエンベロープデータのアドレス
    INC HL
    LD D,(HL)
    LD (IX+12),E					; エンベロープデータのアドレス(下位)
    LD (IX+13),D					; エンベロープデータのアドレス(上位)
    POP DE
    POP HL
...
SOUNDDRV_CHEXEC:
    ...
    CALL SOUNDDRV_SETPSG_VOLENV     ; PSGレジスタ8～10設定
...
SOUNDDRV_CHEXEC_L21:
    ; ■コマンドによる分岐
    OR A                          ; データ=0(休符)か
    JP Z,SOUNDDRV_CHEXEC_CMD0     ; 休符設定処理へ

    CP 219                          ; データ=219(エンベロープ)か
    JP Z,SOUNDDRV_CHEXEC_CMD219     ; エンベロープ値設定処理へ
...
; コマンド：0（休符）設定
; ----------------------------------------------------------------------------------------------------
SOUNDDRV_CHEXEC_CMD0:

    LD (IX+8),0                     ; 休符データ(下位)をワークに保存
    LD (IX+9),0                     ; 休符データ(上位)をワークに保存

    LD A,D                          ; A <- D(トラック番号)
    CP 3
    JR NC,SOUNDDRV_CHEXEC_CMD0_MUTE      ; 1(=SFX)の場合はMUTEへ
...
;   そして次のシーケンスデータの処理を行う
; ----------------------------------------------------------------------------------------------------
SOUNDDRV_CHEXEC_CMD219:
    CALL SOUNDDRV_GETNEXTNATA       ; A <- シーケンスデータ(エンベロープID)
    SLA A                           ; アドレスはワードなので2倍
    LD C, A
    LD B, 0
    LD HL, SOUNDDRV_ENVELOPEWK
    ADD HL, BC						; HL <- 登録したいエンベロープIDのアドレス
    LD C,(HL)						; BC <- エンベロープIDのアドレスから求めたエンベロープデータのアドレス
    INC HL
    LD B,(HL)
    LD (IX+12),C					; エンベロープデータのアドレス(下位)
    LD (IX+13),B					; エンベロープデータのアドレス(上位)
...
;       IX = 対象トラックのワークエリア先頭アドレス
; ----------------------------------------------------------------------------------------------------
SOUNDDRV_SETPSG_VOLENV:
	LD A,(IX+8)
	OR (IX+9)
	RET Z							; (IX+8)と(IX+9)が0:休符（何もせず終了）
    LD C,(IX+11)					; C <- エンベロープカウンタ
    LD B,0
    LD L,(IX+12)					; L <- エンベロープデータのアドレス(下位)
    LD H,(IX+13)					; H <- エンベロープデータのアドレス(上位)
    ADD HL,BC						; HL <- 現在のエンベロープデータのアドレス
    ...
    LD C,A
    LD B,0
    LD HL,SOUNDDRV_VOLTBL			; ボリューム計算テーブルのアドレス
    ADD HL,BC						; 乗算合成(計算めんどくさいのでこれで)
    LD E,(HL)						; A <- 現在のボリューム
...
;          C   C+    D   D+    E    F   F+    G   G+    A   A+    B
	dw  3420,3229,3047,2876,2715,2562,2419,2283,2155,2034,1920,1812 ;o1  1～ 12
	dw  1710,1614,1524,1438,1357,1281,1209,1141,1077,1017, 960, 906 ;o2 13～ 24
	dw   855, 807, 762, 719, 679, 641, 605, 571, 539, 508, 480, 453 ;o3 25～ 36
	dw   428, 404, 381, 360, 339, 320, 302, 285, 269, 254, 240, 226 ;o4 37～ 48
	dw   214, 202, 190, 180, 170, 160, 151, 143, 135, 127, 120, 113 ;o5 49～ 60
	dw   107, 101,  95,  90,  85,  80,  76,  71,  67,  64,  60,  57 ;o6 61～ 72
	dw    53,  50,  48,  45,  42,  40,  38,  36,  34,  32,  30,  28 ;o7 73～ 84
	dw    27,  25,  24,  22,  21,  20,  19,  18,  17,  16,  15,  14 ;o8 85～ 96
...
SOUNDDRV_ENVELOPEWK:
    ...
    DW  $0000                       ; エンベロープデータ4の先頭アドレス
    DW  $0000                       ; エンベロープデータ5の先頭アドレス
    DW  $0000                       ; エンベロープデータ6の先頭アドレス
    DW  $0000                       ; エンベロープデータ7の先頭アドレス
    DW  $0000                       ; エンベロープデータ8の先頭アドレス
    DW  $0000                       ; エンベロープデータ9の先頭アドレス
    DW  $0000                       ; エンベロープデータ10の先頭アドレス
    DW  $0000                       ; エンベロープデータ11の先頭アドレス
    DW  $0000                       ; エンベロープデータ12の先頭アドレス
    DW  $0000                       ; エンベロープデータ13の先頭アドレス
    DW  $0000                       ; エンベロープデータ14の先頭アドレス
    DW  $0000                       ; エンベロープデータ15の先頭アドレス
